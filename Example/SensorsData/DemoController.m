...
- (void)testTrack {
    [[HinaCloudSDK sharedInstance] track:@"testTrack" withProperties:@{@"testName":@"testTrack 测试"}];
}

- (void)testTrackSignup {
    [[HinaCloudSDK sharedInstance] setUserUId:@"newId"];
}

- (void)testTrackInstallation {
    [[HinaCloudSDK sharedInstance] trackAppInstallWithProperties:nil];
}

- (void)testProfileSet {
    [[HinaCloudSDK sharedInstance] set:@"name" to:@"caojiang"];
}

- (void)testProfileAppend {
    [[HinaCloudSDK sharedInstance] append:@"array" by:[NSSet setWithObjects:@"123", nil]];
}

- (void)testProfileIncrement {
    [[HinaCloudSDK sharedInstance] add:@"age" by:@1];
}

- (void)testProfileUnset {
    [[HinaCloudSDK sharedInstance] unset:@"age"];
}
