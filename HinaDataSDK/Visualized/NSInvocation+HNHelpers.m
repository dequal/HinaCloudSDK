...
static void HNFree(void *p) {
    if (p) {
        free(p);
    }
}

static void *HNAllocBufferForObjCType(const char *objCType) {
    void *buffer = NULL;
    ...
    if (result != 0) {
        HNLogError(@"Error allocating aligned memory: %s", strerror(result));
    }
...
@implementation NSInvocation (HNHelpers)
...
- (void)sa_setArgument:(id)argumentValue atIndex:(NSUInteger)index {
    ...
    if ([argumentValue isKindOfClass:[NSNumber class]] && strnlen(argumentType, 8) == 1) {
        ...
        void *buffer = HNAllocBufferForObjCType([valueArgument objCType]);
...
- (id)sa_returnValue {
    ...
    void *buffer = HNAllocBufferForObjCType(objCType);
