...
static NSInteger kHNVisualizedFindMaxPageLevel = 4;
typedef NSArray<UIView *>* (*HNSortedRNSubviewsMethod)(UIView *, SEL);

/// RCTView 响应交互类型
typedef NS_ENUM(NSInteger, HNRCTViewPointerEvents) {
    /// 0: 默认类型，优先使用子视图响应交互
    HNRCTViewPointerEventsUnspecified = 0,
    /// 1: 自身以及子视图都不响应交互，所以不阻塞下层 view 交互
    HNRCTViewPointerEventsNone,
    /// 2: 只让子视图响应交互，自身不可点击
    HNRCTViewPointerEventsBoxNone,
    /// 3: 只有自身接收事件，子视图不可交互
    HNRCTViewPointerEventsBoxOnly,
};

@implementation HNVisualizedUtils

#pragma mark Covered
+ (BOOL)isCoveredForView:(UIView *)view {
    NSArray <UIView *> *allOtherViews = [self findAllPossibleCoverViews:view hierarchyCount:kHNVisualizedFindMaxPageLevel];
...
+ (BOOL)isCoveredOfRNView:(UIView *)view fromRNView:(UIView *)fromView {
     ...
     */
    HNRCTViewPointerEvents pointerEvents = [self pointEventsWithRCTView:fromView];
    // RCTView 重写 hitTest: 并返回 nil，不阻塞底下元素交互
    if (pointerEvents == HNRCTViewPointerEventsNone) {
        return NO;
    }
    // 遍历子视图判断是否存在坐标覆盖阻塞交互
    if (pointerEvents == HNRCTViewPointerEventsBoxNone) {
        // 寻找完全遮挡 view 的子视图
        for (UIView *subView in fromView.subviews) {
            BOOL enableInteraction = [HNVisualizedUtils isVisibleForView:subView] && subView.userInteractionEnabled;
...
+ (NSArray *)findPossibleCoverAllBrotherViews:(UIView *)view {
    ...
    if ([self isKindOfRCTView:superView]) {
        /* RCTView 默认重写了 hitTest:
         ...
         详情参照：https://github.com/facebook/react-native/blob/master/React/Views/RCTView.m
         */
        HNRCTViewPointerEvents pointerEvents = [self pointEventsWithRCTView:superView];
        // RCTView 重写 hitTest: 并返回 nil，不阻塞底下元素交互
        if (pointerEvents != HNRCTViewPointerEventsNone && pointerEvents != HNRCTViewPointerEventsBoxOnly) {
...
+ (NSArray *)analysisWebElementWithWebView:(WKWebView <HNVisualizedExtensionProperty> *)webView {
...
+ (NSArray<UIView *> *)sortedRNSubviewsWithView:(UIView *)view {
    ...
    HNSortedRNSubviewsMethod method = (HNSortedRNSubviewsMethod)[view methodForSelector:sortedRNSubviewsSel];
...
+ (BOOL)isInteractiveEnabledRNView:(UIView *)view {
    ...
    if (view.superview.hinadata_isDisableRNSubviewsInteractive) {
        view.hinadata_isDisableRNSubviewsInteractive = YES;
        return NO;
    }
    ...
    // 设置交互状态
    HNRCTViewPointerEvents pointerEvents = [self pointEventsWithRCTView:view];
    // None 和 BoxOnly 都禁用子视图交互
    BOOL isEventsNone = pointerEvents == HNRCTViewPointerEventsNone;
    BOOL isEventsBoxOnly = pointerEvents == HNRCTViewPointerEventsBoxOnly;
    view.hinadata_isDisableRNSubviewsInteractive = isEventsNone || isEventsBoxOnly;
    
    // EventsNone 和 EventsBoxNone 时，自身不可交互
    if (pointerEvents == HNRCTViewPointerEventsNone || pointerEvents == HNRCTViewPointerEventsBoxNone) {
        return NO;
    }
...
/// 获取当前 RCTView 的交互类型
+ (HNRCTViewPointerEvents)pointEventsWithRCTView:(UIView *)view {
    if (![self isKindOfRCTView:view]) {
        return HNRCTViewPointerEventsUnspecified;
    }
    
    HNRCTViewPointerEvents pointerEvents = HNRCTViewPointerEventsUnspecified;
    @try {
        pointerEvents = [[view valueForKey:@"pointerEvents"] integerValue];
    } @catch (NSException *exception) {
        HNLogWarn(@"%@ error: %@", self, exception);
    }
...
+ (NSArray *)analysisFlutterElementWithFlutterView:(UIView *)flutterView {
    HNVisualizedPageInfo *flutterPageInfo = [[HNVisualizedObjectSerializerManager sharedInstance] queryPageInfoWithType:HNVisualizedPageTypeFlutter];
...
+ (NSArray *)buildVisualizedElementsWithVisualizedPageInfo:(HNVisualizedPageInfo *)pageInfo superView:(UIView *)suerperView {
    ...
    NSMutableArray<HNVisualizedElementView *> *webElementArray = [NSMutableArray array];
    ...
    for (NSDictionary *pageData in elementSources) {
        ...
        HNVisualizedElementView *element = [self visualizedElementWithInfo:pageData superView:suerperView pagetype:pageInfo.pageType];
        if (element) {
    ...
    // 根据 level 升序排序
    [webElementArray sortUsingComparator:^NSComparisonResult(HNWebElementView *obj1,HNWebElementView *obj2) {
    ...
    // 构建子元素数组
    for (HNWebElementView *webElement1 in [webElementArray copy]) {
        //当前元素是否嵌套子元素
        ...
        // 根据子元素 id 查找对应子元素
        for (HNWebElementView *webElement2 in [webElementArray copy]) {
...
+ (HNVisualizedElementView *)visualizedElementWithInfo:(NSDictionary *)elementInfo superView:(UIView *)superView pagetype:(HNVisualizedPageType)type {
    if (type == HNVisualizedPageTypeWeb) {
        if (![superView isKindOfClass:WKWebView.class]) {
            return nil;
        }
        WKWebView *webView = (WKWebView *)superView;
        return [[HNWebElementView alloc] initWithWebView:webView webElementInfo:elementInfo];
    }
    if (type == HNVisualizedPageTypeFlutter) {
        return [[HNFlutterElementView alloc] initWithSuperView:superView elementInfo:elementInfo];
    }
    return nil;
}
...
+ (UIImage *)screenshotWithView:(UIView *)view {
    ...
    @try {
        ...
        HNLogError(@"screenshot fail，error %@: %@", self, exception);
    }
...
+ (BOOL)isSupportCallJSWithWebView:(WKWebView *)webview {
    ...
    [userScripts enumerateObjectsUsingBlock:^(WKUserScript *_Nonnull obj, NSUInteger idx, BOOL *_Nonnull stop) {
        if ([obj.source containsString:kHNJSBridgeServerURL]) {
...
@implementation HNVisualizedUtils (ViewPath)

+ (BOOL)isIgnoredViewPathForViewController:(UIViewController *)viewController {
    BOOL isEnableVisualized =  [[HNVisualizedManager defaultManager] isVisualizeWithViewController:viewController];
...
+ (NSString *)viewSimilarPathForView:(UIView *)view atViewController:(UIViewController *)viewController {
    ...
    return [HNUIProperties elementPathForView:view atViewController:viewController];
}
