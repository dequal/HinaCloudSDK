...
@implementation HNViewIdentifier
...
- (instancetype)initWithView:(UIView *)view {
    self = [super init];
    if (self) {
        _elementPath = view.hinadata_elementPath;
        _screenName = view.hinadata_screenName;
        _elementContent = view.hinadata_elementContent;
        _elementPosition = view.hinadata_elementPosition;
        _pageIndex = [HNVisualizedUtils pageIndexWithView:view];
    }
    return self;
}
...
@implementation HNVisualPropertiesEventConfig
...
@implementation HNVisualPropertiesPropertyConfig
...
- (instancetype)initWithDictionary:(NSDictionary *)propertiesDic {
    ...
    if (self) {
        ...
        if ([type isEqualToString:@"NUMBER"]) {
            _type = HNVisualPropertyTypeNumber;
        } else {
            _type = HNVisualPropertyTypeString;
        }
...
/// h5 属性配置，匹配元素
- (BOOL)isEqualToWebViewIdentify:(HNViewIdentifier *)object {
    if (!self.isH5) {
        return NO;
    }
...
@implementation HNVisualPropertiesConfig
...
- (instancetype)initWithDictionary:(NSDictionary *)eventsDic {
    ...
    if (self) {
        ...
        if ([eventTypeString isEqualToString:@"appclick"]) {
            _eventType = HinaDataEventTypeAppClick;
        } else {
            _eventType = HinaDataEventTypeNone;
        }

        _eventName = dictionaryValueForKey(eventsDic, @"event_name");
        NSDictionary *eventDic = dictionaryValueForKey(eventsDic, @"event");
        _event = [[HNVisualPropertiesEventConfig alloc] initWithDictionary:eventDic];

        NSArray<NSDictionary *> *propertiesArray = dictionaryValueForKey(eventsDic, @"attr");
        if (propertiesArray) {

            NSMutableArray<HNVisualPropertiesPropertyConfig *> *properties = [NSMutableArray array];
            NSMutableArray <NSDictionary *>*webProperties = [NSMutableArray array];
            for (NSDictionary *dic in propertiesArray) {
                HNVisualPropertiesPropertyConfig *config = [[HNVisualPropertiesPropertyConfig alloc] initWithDictionary:dic];
...
- (void)encodeWithCoder:(NSCoder *)coder {
    ...
    [coder encodeObject:self.properties forKey:@"attr"];
...
- (instancetype)initWithCoder:(NSCoder *)coder {
    ...
    if (self) {
        ...
        self.properties = [coder decodeObjectForKey:@"attr"];
...
@implementation HNVisualPropertiesResponse
    ...
    if (self) {
        ...
        if (events.count > 0) {
            ...
            for (NSDictionary *eventDic in events) {
                HNVisualPropertiesConfig *event = [[HNVisualPropertiesConfig alloc] initWithDictionary:eventDic];
