...
- (void)buildNodeRelation {
    ...
    for (UIView *subView in view.subviews) {
        if (subView.hinadata_viewNode) {
            // 可能存在其他父节点，进行移除
            if (subView.hinadata_viewNode.nextNode && subView.hinadata_viewNode.nextNode != self) {
                [subView.hinadata_viewNode removeOldNodeRelation];
            }
            [subView.hinadata_viewNode buildNextNodeRelationWithNextNode:self];
        }
    }
    ...
    HNViewNode *nextNode = nextView.hinadata_viewNode;
    if (nextNode) {
...
- (void)buildNextNodeRelationWithNextNode:(HNViewNode *)nextNode {
    // 链接父节点
...
- (void)removeOldNodeRelation {
    HNViewNode *oldNextNode = self.nextNode;
...
- (void)refreshIndex {
    ...
    NSInteger index = 0;
    for (HNViewNode *node in self.nextNode.subNodes) {
        if (node == self) {
            self.index = index;
            return;
        }
...
- (void)refreshScreenName {
    [HNCommonUtility performBlockOnMainThread:^{
        self.screenName = [self.view hinadata_screenName];
    }];
}
...
- (void)refreshSubNodeScreenName {
    ...
    for (HNViewNode *subNode in self.subNodes) {
...
- (NSString *)elementPosition {
    HNViewNode *nextNode = self.nextNode;
...
- (NSString *)elementPath {
    ...
    可以在子线程执行
    实现参考 [HNVisualizedUtils viewSimilarPathForView: atViewController:]
     */
    HNViewNode *currentNode = self;
...
- (NSString *)elementPath {
    HNViewNode *segmentedControlNode = self.nextNode;
...
@interface HNSegmentedControlNode()
@property (nonatomic, strong) UISegmentedControl *segmentedControl;
@end

@implementation HNSegmentedControlNode
...
@implementation HNTabBarButtonNode
...
@implementation HNTableViewHeaderFooterViewNode
...
@interface HNCellNode()
@property (nonatomic, strong) NSIndexPath *indexPath;

@end

@implementation HNCellNode
...
-(NSIndexPath *)indexPath {
    if (!_indexPath) {
        [HNCommonUtility performBlockOnMainThread:^{
            if ([HNVisualizedUtils isVisibleForView:self.view]) {
                [self refreshIndex];
            }
        }];
    }
...
- (NSString *)similarPath {
    if (self.indexPath) {
        // 弹框或 menu 等，不支持限定位置
        HNViewElementInfo *elementInfo = [HNViewElementInfoFactory elementInfoWithView:self.view];
...
@implementation HNRNViewNode
- (instancetype)initWithView:(UIView *)view {
    [HNRNViewNode bindScreenNameWithClickableView:view];
    self = [super initWithView:view];
    return self;
}
...
+ (void)bindScreenNameWithClickableView:(UIView *)rnView {
    [rnView hinadata_clickableForRNView];
}

@end


@implementation HNWKWebViewNode
...
- (void)callJSSendVisualConfig:(NSDictionary *)configResponse {
    ...
    // 判断打通才注入配置
    if (![HNVisualizedUtils isSupportCallJSWithWebView:webView]) {
        return;
    }
    // 调用 JS 函数，发送配置信息
    NSString *javaScriptSource = [HNJavaScriptBridgeBuilder buildCallJSMethodStringWithType:HNJavaScriptCallJSTypeUpdateVisualConfig jsonObject:configResponse];
    if (!javaScriptSource) {
        return;
    }
    [webView evaluateJavaScript:javaScriptSource completionHandler:^(id _Nullable resuts, NSError * _Nullable error) {
        if (error) {
            HNLogDebug(@"%@ updateH5VisualConfig error: %@", kHNJSBridgeCallMethod, error);
        } else {
            HNLogDebug(@"%@ updateH5VisualConfig finish", kHNJSBridgeCallMethod);
        }
    }];
}
...
@implementation HNIgnorePathNode
