...
+ (NSData *)gzipData:(NSData *)pUncompressedData {
    ...
    if (!pUncompressedData || [pUncompressedData length] == 0) {
        HNLogError(@" Error: Can't compress an empty or null NSData object.");
        return nil;
    }
    ...
    if (initError != Z_OK) {
        ...
        HNLogError(@"deflateInit2() Error: \"%@\" Message: \"%s\"", errorMsg, zlibStreamStruct.msg);
        return nil;
    }
    ...
    if (deflateStatus != Z_STREAM_END) {
        ...
        HNLogError(@"zlib error while attempting compression: \"%@\" Message: \"%s\"", errorMsg, zlibStreamStruct.msg);
