...
+ (NSURL *)buildServerURLWithURLString:(NSString *)urlString debugMode:(HinaDataDebugMode)debugMode {
    ...
    if (debugMode != HinaDataDebugOff) {
        ...
        if (serverURL.host && [serverURL.host rangeOfString:@"_"].location != NSNotFound) { //包含下划线日志提示
            ...
            HNLogWarn(@"Server url:%@ contains '_'  is not recommend,see details:%@", serverURL, referenceURL);
        }
    }
