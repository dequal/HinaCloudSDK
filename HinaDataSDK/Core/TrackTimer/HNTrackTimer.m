...
- (NSString *)generateEventIdByEventName:(NSString *)eventName {
    ...
    if (![eventName hasSuffix:kHNEventIdSuffix]) {
        //生成计时事件的 eventId，结构为 {eventName}_{uuid}_HNTimer
        //uuid 字符串中 ‘-’ 是不合法字符，替换为 ‘_’
        NSString *uuid = [NSUUID.UUID.UUIDString stringByReplacingOccurrencesOfString:@"-" withString:@"_"];
        eventId = [NSString stringWithFormat:@"%@_%@%@", eventName, uuid, kHNEventIdSuffix];
    }
    return eventId;
}

- (void)trackTimerStart:(NSString *)eventId currentSysUpTime:(UInt64)currentSysUpTime {
    return [self trackTimerStart:eventId timeUnit:HinaDataTimeUnitSeconds currentSysUpTime:currentSysUpTime];
}

- (void)trackTimerStart:(NSString *)eventId timeUnit:(HinaDataTimeUnit)timeUnit currentSysUpTime:(UInt64)currentSysUpTime {
...
- (NSString *)eventNameFromEventId:(NSString *)eventId {
    if (![eventId hasSuffix:kHNEventIdSuffix]) {
        return eventId;
    }
...
- (NSNumber *)handleEventDuration:(NSString *)eventId mapping:(NSMutableDictionary *)mapping currentSystemUpTime:(UInt64)currentSystemUpTime {
    ...
    HinaDataTimeUnit timeUnit = [[eventTimer valueForKey:@"timeUnit"] intValue];
...
- (void)handleAllEventsPause:(NSMutableDictionary *)mapping time:(UInt64)currentSystemUpTime {
    for (NSString *key in mapping.allKeys) {
        if (key != nil) {
            if ([key isEqualToString:kHNEventNameAppEnd]) {
                continue;
            }
        }
...
//计算事件时长
- (float)eventTimerDurationWithCurrentTime:(UInt64)currentSystemUpTime eventStart:(UInt64)startTime timeUnit:(HinaDataTimeUnit)timeUnit {
    if (startTime <= 0) {
        return 0;
    }
    ...
    if (eventDuration > 0 && eventDuration < 24 * 60 * 60 * 1000) {
        switch (timeUnit) {
            case HinaDataTimeUnitHours:
                eventDuration = eventDuration / 60.0;
            case HinaDataTimeUnitMinutes:
                eventDuration = eventDuration / 60.0;
            case HinaDataTimeUnitSeconds:
                eventDuration = eventDuration / 1000.0;
            case HinaDataTimeUnitMilliseconds:
                break;
        }
    } else {
        eventDuration = 0;
