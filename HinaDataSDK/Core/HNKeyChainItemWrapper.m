...
NSString * const kHNService = @"com.hinadata.analytics.udid";
NSString * const kHNUdidAccount = @"com.hinadata.analytics.udid";

@implementation HNKeyChainItemWrapper
+ (NSString *)saUdid {
    NSDictionary *result = [self fetchPasswordWithAccount:kHNUdidAccount service:kHNService];
    NSString *udid = [result objectForKey:(__bridge id)kSecValueData];
    return udid;
}

+ (NSString *)saveUdid:(NSString *)udid {
    BOOL sucess = [self saveOrUpdatePassword:udid account:kHNUdidAccount service:kHNService];
...
+ (BOOL)saveOrUpdatePassword:(NSString *)password account:(NSString *)account service:(NSString *)service accessGroup:(NSString *)accessGroup {
    @try {
        ...
        if (status == errSecSuccess) {
            ...
            HNLogDebug(@"SecItemUpdate result = %d", result);
            ...
            HNLogDebug(@"SecItemAdd result = %d", status);
        }
