...
void *HNAppExtensionQueueTag = &HNAppExtensionQueueTag;

@interface HNAppExtensionDataManager() {
}
@property (nonatomic, strong) dispatch_queue_t appExtensionQueue;
@end

@implementation HNAppExtensionDataManager

+ (instancetype)sharedInstance {
    static HNAppExtensionDataManager *manager = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        manager = [[HNAppExtensionDataManager alloc] init];
    });
    return manager;
}
...
- (instancetype)init {
    if (self = [super init]) {
        self.appExtensionQueue = dispatch_queue_create("com.hinadata.analytics.appExtensionQueue", DISPATCH_QUEUE_SERIAL);
        dispatch_queue_set_specific(self.appExtensionQueue, HNAppExtensionQueueTag, &HNAppExtensionQueueTag, NULL);
    }
    return self;
}
...
- (void)setGroupIdentifierArray:(NSArray *)groupIdentifierArray {
    ...
    if (dispatch_get_specific(HNAppExtensionQueueTag)) {
        block();
    } else {
...
- (NSString *)filePathForApplicationGroupIdentifier:(NSString *)groupIdentifier {
    @try {
        ...
        dispatch_block_t block = ^() {
            NSURL *pathUrl = [[[NSFileManager defaultManager] containerURLForSecurityApplicationGroupIdentifier:groupIdentifier] URLByAppendingPathComponent:@"hina_event_data.plist"];
...
- (BOOL)writeEvent:(NSString *)eventName properties:(NSDictionary *)properties groupIdentifier:(NSString *)groupIdentifier {
    @try {
        ...
        dispatch_block_t block = ^{
            NSDictionary *event = @{@"event": eventName, @"attr": properties?properties:@{}};
            NSString *path = [self filePathForApplicationGroupIdentifier:groupIdentifier];
