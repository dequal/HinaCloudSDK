...
@interface HNRSAEncryptor ()

@end

@implementation HNRSAEncryptor
...
- (NSString *)algorithm {
    return kHNAlgorithmTypeRSA;
}

- (NSString *)encryptData:(NSData *)data {
    if (![HNValidator isValidData:data]) {
        HNLogError(@"Enable RSA encryption but the input obj is invalid!");
        return nil;
    }

    NSString *asymmetricPublicKey = self.key;
    if (![HNValidator isValidString:asymmetricPublicKey]) {
        HNLogError(@"Enable RSA encryption but the public key is invalid!");
        return nil;
    }
    
    SecKeyRef keyRef = [self addPublicKey:asymmetricPublicKey];
    if (!keyRef) {
        HNLogError(@"Enable RSA encryption but init public SecKeyRef failed!");
        return nil;
    }
    ...
    for(int idx=0; idx<srclen; idx+=src_block_size) {
        ...
        if (status != 0) {
            HNLogError(@"SecKeyEncrypt fail. Error Code: %d", (int)status);
            ret = nil;
            break;
        }else{
...
- (SecKeyRef)addPublicKey:(NSString *)aymmetricPublicKey {
    ...
    NSString *tag = @"Hina_RSAUtil_PubKey";
