...
@interface HNBuildOptions ()<NSCopying>

@property (atomic, strong, readwrite) NSMutableArray *encryptors;
@property (nonatomic, assign) BOOL enablePushAutoTrack;
...
- (instancetype)initWithServerURL:(NSString *)serverURL launchOptions:(id)launchOptions {
    ...
    if (self) {
        ...
        _autoTrackEventType = HNAutoTrackNone;
...
- (id)copyWithZone:(nullable NSZone *)zone {
    HNBuildOptions *options = [[[self class] allocWithZone:zone] init];
    options.serverURL = self.serverURL;
    options.launchOptions = self.launchOptions;
    options.enableJSBridge = self.enableJSBridge;
    options.flushInterval = self.flushInterval;
    options.flushPendSize = self.flushPendSize;
    ...
    // 推送点击
    options.enablePushAutoTrack = self.enablePushAutoTrack;
    // 页面浏览时长
...
- (void)setFlushPendSize:(NSInteger)flushPendSize {
    _flushPendSize = flushPendSize >= 50 ? flushPendSize : 50;
}
