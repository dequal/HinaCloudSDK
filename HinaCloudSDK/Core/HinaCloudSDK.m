...
static HinaCloudSDK *sharedInstance = nil;
...
@implementation HinaCloudSDK

#pragma mark - Initialization
+ (void)startWithConfigOptions:(HNBuildOptions *)configOptions {
    ...
    dispatch_once(&sdkInitializeOnceToken, ^{
        sharedInstance = [[HinaCloudSDK alloc] initWithConfigOptions:configOptions];
...
+ (HinaCloudSDK *)sdkInstance {
    return sharedInstance;
}

+ (void)disableSDK {
    HinaCloudSDK *instance = HinaCloudSDK.sdkInstance;
    ...
    SALogWarn(@"HinaCloudSDK disabled");
...
+ (void)enableSDK {
    ...
    SALogInfo(@"HinaCloudSDK enabled");
}

- (instancetype)initWithConfigOptions:(nonnull HNBuildOptions *)configOptions {
    @try {
        self = [super init];
        if (self) {
...
#if TARGET_OS_IOS
- (void)setupSecurityPolicyWithConfigOptions:(HNBuildOptions *)options {
...
- (void)resetAnonymousId {
    dispatch_async(self.serialQueue, ^{
        NSString *previousAnonymousId = [self.deviceUId copy];
        [self.identifier resetAnonymousId];
        if (self.loginId || [previousAnonymousId isEqualToString:self.deviceUId]) {
            return;
        }
...
- (void)set:(NSDictionary *)userDict {
    if (userDict) {
        [self profile:kSAProfileSet properties:userDict];
    }
}

- (void)setOnce:(NSDictionary *)userDict {
    if (userDict) {
        [self profile:kSAProfileSetOnce properties:userDict];
    }
}
...
- (void)unset:(NSString *) user {
    if (user) {
        [self profile:kSAProfileUnset properties:@{user: @""}];
    }
}
...
- (void)add:(NSDictionary *)userDict {
    if (userDict) {
        ...
        [self trackEventObject:object properties:userDict];
    }
}
...
@implementation HinaCloudSDK (Deprecated)
...
- (NSInteger)flushPendSize {
    @synchronized(self) {
        return self.configOptions.flushPendSize;
    }
}

- (void)setFlushPendSize:(NSInteger)bulkSize {
    @synchronized(self) {
        self.configOptions.flushPendSize = bulkSize;
    }
}
...
- (void)setFlushNetworkPolicy:(HNNetworkType)networkType {
    @synchronized (self) {
