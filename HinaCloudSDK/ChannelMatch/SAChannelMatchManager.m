...
- (void)setConfigOptions:(HNBuildOptions *)configOptions {
    ...
    [HinaCloudSDK.sharedInstance registerPropertyPlugin:channelInfoPropertyPlugin];
}
...
- (void)trackAppInstall:(NSString *)event properties:(NSDictionary *)properties disableCallback:(BOOL)disableCallback{
    ...
    [HinaCloudSDK.sharedInstance trackEventObject:eventObject properties:eventProps];
    ...
    [HinaCloudSDK.sharedInstance trackEventObject:profileObject properties:profileProps];
}
...
- (void)trackChannelWithEventObject:(SABaseEventObject *)obj properties:(nullable NSDictionary *)propertyDict {
    if (self.configOptions.enableAutoAddChannelCallbackEvent) {
        return [HinaCloudSDK.sharedInstance trackEventObject:obj properties:propertyDict];
    }
    ...
    [HinaCloudSDK.sharedInstance trackEventObject:obj properties:properties];
}
...
- (BOOL)handleURL:(NSURL *)url {
    ...
    SANetwork *network = [HinaCloudSDK sharedInstance].network;
...
- (void)uploadUserInfoIntoWhiteList:(NSDictionary *)qureyItems {
    ...
    NSURLComponents *components = HinaCloudSDK.sharedInstance.network.baseURLComponents;
    if (!components) {
        return;
    }
    ...
    params[@"distinct_id"] = [[HinaCloudSDK sharedInstance] distinctId];
...
- (void)showChannelDebugInstall {
    ...
    [alertController addActionWithTitle:SALocalizedString(@"SAChannelActivate") style:SAAlertActionStyleDefault handler:^(SAAlertAction * _Nonnull action) {
        dispatch_queue_t serialQueue = HinaCloudSDK.sharedInstance.serialQueue;
        // 入队列前，执行动态公共属性采集 block
        [HinaCloudSDK.sharedInstance buildDynamicSuperProperties];
