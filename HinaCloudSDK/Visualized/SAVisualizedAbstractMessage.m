...
- (NSData *)JSONDataWithFeatureCode:(NSString *)featureCode {
    ...
    SensorsAnalyticsAutoTrackEventType eventType = HinaCloudSDK.sharedInstance.configOptions.autoTrackEventType;
    if (eventType &  HNAutoTrackAppClick) {
        [autotrackOptions addObject:kSAEventNameAppClick];
    }
    if (eventType &  HNAutoTrackAppScreen) {
    ...
    // SDK 版本号
    jsonObject[@"lib_version"] = HinaCloudSDK.sharedInstance.libVersion;
    // 可视化全埋点配置版本号
