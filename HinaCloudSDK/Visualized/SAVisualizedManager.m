...
- (BOOL)handleURL:(NSURL *)url {
    ...
    BOOL isEqualProject = [[HinaCloudSDK sharedInstance].network.project isEqualToString:project];
    if (!isEqualProject) {
    ...
    // 未开启点击图
    if ([url.host isEqualToString:@"heatmap"] && ![[HinaCloudSDK sharedInstance] isHeatMapEnabled]) {
    ...
    // 未开启可视化全埋点
    if ([url.host isEqualToString:@"visualized"] && ![[HinaCloudSDK sharedInstance] isVisualizedAutoTrackEnabled]) {
