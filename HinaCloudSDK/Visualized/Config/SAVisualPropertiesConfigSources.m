...
- (NSURLRequest *)buildConfigRequest {
    ...
    if (!components) {
        NSString *logMessage = [SAVisualizedLogger buildLoggerMessageWithTitle:@"get configuration" message:@"serverURL is invalid: %@", HinaCloudSDK.sharedInstance.network.serverURL];
    ...
    params[@"project"] = HinaCloudSDK.sharedInstance.network.project;
...
- (void)unarchiveConfig {
    NSString *project = HinaCloudSDK.sharedInstance.network.project;
...
- (nullable NSArray <SAVisualPropertiesConfig *> *)propertiesConfigsWithViewNode:(SAViewNode *)viewNode {
    ...
    for (SAVisualPropertiesConfig *config in configSources) {
        ...
        // 命中配置信息
        if (config.eventType == HNAutoTrackAppClick && [config.event isMatchVisualEventWithViewIdentify:viewNode]) {
...
- (nullable NSArray <SAVisualPropertiesConfig *> *)propertiesConfigsWithEventIdentifier:(SAEventIdentifier *)eventIdentifier {
    ...
    for (SAVisualPropertiesConfig *config in configSources) {
        // 命中 AppClick 配置
        if (config.eventType == HNAutoTrackAppClick && [config.event isMatchVisualEventWithViewIdentify:eventIdentifier]) {
            [configs addObject:config];
        }
    }
