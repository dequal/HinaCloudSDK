...
#import "HNBuildOptions+RemoteConfig.h"

#if __has_include("HNBuildOptions+Encrypt.h")
#import "HNBuildOptions+Encrypt.h"
#endif
...
- (instancetype)initWithConfigOptions:(HNBuildOptions *)configOptions remoteConfigModel:(SARemoteConfigModel *)model {
    self = [super init];
    if (self) {
...
- (void)trackAppRemoteConfigChanged:(NSDictionary<NSString *, id> *)remoteConfig {
    ...
    [HinaCloudSDK.sdkInstance trackEventObject:object properties:@{kSAEventPropertyAppRemoteConfig : eventConfigString ?: @""}];
    // 触发 $AppRemoteConfigChanged 时 flush 一次
    [HinaCloudSDK.sdkInstance flush];
}
...
- (BOOL)isLibVersionUnchanged {
    return [self.model.localLibVersion isEqualToString:HinaCloudSDK.sdkInstance.libVersion];
}
