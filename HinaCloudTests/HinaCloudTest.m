...
@interface HinaCloudTest : XCTestCase

@end

@implementation HinaCloudTest

- (void)setUp {
    // Put setup code here. This method is called before the invocation of each test method in the class.
    HNBuildOptions *options = [[HNBuildOptions alloc] initWithServerURL:@"http://sdk-test.cloud.sensorsdata.cn:8006/sa?project=default&token=95c73ae661f85aa0" launchOptions:nil];
...
- (void)testProfileEventWithoutCallback {
    ...
    [[HinaCloudSDK sharedInstance] trackEventCallback:^BOOL(NSString * _Nonnull eventName, NSMutableDictionary<NSString *,id> * _Nonnull properties) {
        isTrackEventCallbackExecuted = YES;
        return YES;
    }];
    [[HinaCloudSDK sharedInstance] set:@"avatar_url" to:@"http://www.sensorsdata.cn"];
    sleep(0.5);
...
//       
//       NSInteger lastCount = [HinaCloudSDK sharedInstance].messageQueue.count;
//       [[HinaCloudSDK sharedInstance] itemSetWithType:@"itemSet0517" itemId:@"itemId0517" properties:@{@"itemSet":@"acsdfgvzscd"}];
//       
//       sleep(1);
//       
//       NSInteger newCount = [HinaCloudSDK sharedInstance].messageQueue.count;
...
//   dispatch_async(queue, ^{
//       NSInteger lastCount = [HinaCloudSDK sharedInstance].messageQueue.count;
//       [[HinaCloudSDK sharedInstance] itemDeleteWithType:@"itemSet0517" itemId:@"itemId0517"];
//
//       sleep(1);
//
//       NSInteger newCount = [HinaCloudSDK sharedInstance].messageQueue.count;
...
- (void)testShouldTrackTimerStart {
    ...
    dispatch_async(queue, ^{
        ...
        [[HinaCloudSDK sharedInstance] trackTimerStart:@"timerEvent"];
        sleep(1);
        [[HinaCloudSDK sharedInstance] trackTimerEnd:@"timerEvent"];
    });
...
- (void)testTrackTimerPause {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.5 * NSEC_PER_SEC)), queue, ^{
        [[HinaCloudSDK sharedInstance] trackTimerPause:@"timerEvent"];
    });
...
- (void)testTrackTimerResume {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(3.5 * NSEC_PER_SEC)), queue, ^{
        [[HinaCloudSDK sharedInstance] trackTimerResume:@"timerEvent"];
    });
...
//测试动态开启 HNAutoTrackAppEnd，event_duration 是否从启动开始计算
...
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
        [[HinaCloudSDK sharedInstance] enableAutoTrack:HNAutoTrackAppEnd];
#pragma clang diagnostic pop
        ...
        [[HinaCloudSDK sharedInstance] track:@"$AppEnd"];
    });
